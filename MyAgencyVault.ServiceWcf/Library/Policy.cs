...
namespace MyAgencyVault.WcfService
{
    ...
    interface IPolicy
    {
        ...
        [OperationContract]
        PolicySavedStatus SavePolicy(PolicyDetailsData Policy, PolicyDetailsData ReplacedPolicy, string strRenewal,string strCoverageNickName, Guid? CurrentUserID);
        ...
        [OperationContract]
        void SavePolicyLastUpdated(Guid PolicyID, Guid? CurrentUserID);

        [OperationContract]
        string CalculatePolicyType(DateTime? OriginalEffectiveDate, Guid? ClientId, Guid PolicyLicenseeId, Guid PolicyId, Guid? CoverageId);

        //[OperationContract]
        //void setIsManuallyChangedPolicyTypeFunctionCalled(bool IsManuallyChanged, bool IsPolicyTypeFunctionCalled);//seeA

        //[OperationContract]
        //bool? getValueOfGetIsManuallyChanged();//seeA
    }
    ...
    public partial class MavService : IPolicy
    {
        ...
        public void SavePolicyLastUpdated(Guid PolicyID, Guid? CurrentUserID)
        {
            Policy.ModifyPolicyLastUpdated(PolicyID, CurrentUserID);
        }
        public PolicySavedStatus SavePolicy(PolicyDetailsData PolicySave, PolicyDetailsData ReplacedPolicy, string strRenewal,string strCoverageNickName, Guid? CurrentUserID)
        {
            return Policy.SavePolicy(PolicySave, ReplacedPolicy, strRenewal, strCoverageNickName, CurrentUserID);
        }
        ...
        public string CalculatePolicyType(DateTime? OriginalEffectiveDate, Guid? ClientId, Guid PolicyLicenseeId, Guid PolicyId, Guid? CoverageId)
        {
            string PolicyType = "";
            PolicyType = Policy.calculatePolicyType(OriginalEffectiveDate, ClientId, PolicyLicenseeId, PolicyId, CoverageId);
            return PolicyType;
        }

        //public void setIsManuallyChangedPolicyTypeFunctionCalled(bool IsManuallyChanged, bool IsPolicyTypeFunctionCalled)//seeA
        //{
        //    Policy.setIsManuallyChangedPolicyTypeFunctionCalled(IsManuallyChanged, IsPolicyTypeFunctionCalled);
        //}

        //public bool? getValueOfGetIsManuallyChanged()//seeA
        //{
        //    bool? getIsManuallyChanged = Policy.getValueOfGetIsManuallyChanged();
