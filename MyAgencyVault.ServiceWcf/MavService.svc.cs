...
namespace MyAgencyVault.WcfService
{
    ...
    public partial class MavService : IMavService, IErrorHandler
    {
        ...
        {
            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " GetServerURL version : " + appVersion, true);
                ...
                using (var stringWriter = new StringWriter(builder))
                {
                    using (var xmlReader = new XmlTextReader(url))
                    {
                        ...
                        if (version.Number > Convert.ToInt16(appVersion))
                        {
                            ...
                            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " GetServerURL version found old ", true);
                        }
                        else
                        {
                            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " GetServerURL version found new ", true);
                        }
                    }
                }
            }
            ...
            {
                ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " GetServerURL exception : " + ex.Message, true);
            }
            return obj;
        }
