    ...
    public class VMCompManager : BaseViewModel, IDataRefresh
    {
        ...
        void VMCompManager_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            switch (e.PropertyName)
            {
                ...
                case "SelectedBatchDate":
                    if (SharedVMData.SelectedLicensee.LicenseeId != Guid.Empty)
                    {
                        BatchLst = new ObservableCollection<Batch>(serviceClients.BatchClient.GetCurrentBatch(SharedVMData.SelectedLicensee.LicenseeId, SelectedBatchDate).OrderByDescending(date => date.CreatedDate).ThenByDescending(batch => batch.BatchNumber).ToList());
                        SelectedBatch = BatchLst.FirstOrDefault();
                    }
                    break;               
                ...
                case "SetClientByLinkedDialog":
                    ...
                    serviceClients.LinkPaymentPoliciesClient.MakePolicyActive(LinkPaymentSelectedPendingPolicies.PolicyId, SetClientByLinkedDialog.ClientId);
        ...
        public void SelectedLicenseeChanged()
        {
            ...
            if (SharedVMData.SelectedLicensee.LicenseeId != Guid.Empty)
            {
                ...
                BatchLst = new ObservableCollection<Batch>(serviceClients.BatchClient.GetCurrentBatch(SharedVMData.SelectedLicensee.LicenseeId, SelectedBatchDate).OrderByDescending(o => o.CreatedDate).ThenByDescending(o => o.BatchNumber).ToList());
                if (BatchLst != null && BatchLst.Count != 0)
                {
                    SelectedBatch = BatchLst.FirstOrDefault();
                }
        ...
        private void UploadBatchFile(string strFilePath)
        {
            ...
            try
            {
                ...
                if (strFileExtension.Contains(".pdf"))
                {
                    ...
                    BatchLst = new ObservableCollection<Batch>(BatchLst.OrderByDescending(p => p.BatchNumber));
                }
