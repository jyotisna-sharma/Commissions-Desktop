...
namespace MyAgencyVault.VM.CommonItems
{
  ...
  public delegate void DownloadCompleteDel(int statusCode, string localFilePath);
  ...
  public class FileUtility
  {
    ...
    public static FileUtility CreateClient(string server, string UserName, string password, string domain)
    {
      ...
      return dav;
       ...
       // public static FileUtility CreateClientlocal(string server, string UserName, string password, string domain)
       //{
       //    FileUtility dav = new FileUtility();
       //   dav.Server = "http://dev.commissiondept.com/FileManager";
       //   dav.User = "comm-Dev";
       //   dav.Pass = "c0mm@d3v";
       //   dav.Domain = "comm-Dev";
       //   return dav;
    ...
    public void DownloadFile(String remoteFilePath, String localFilePath, VMVersionHelper helper)
    {
      _VMVersionHelper = helper;
      Download(remoteFilePath, localFilePath);
    }
    private VMVersionHelper _VMVersionHelper;
    void FinishDownload(IAsyncResult result)
    {
      try
      {
          ...
          using (Stream s = response.GetResponseStream())
          {
            using (FileStream fs = new FileStream(localFilePath, FileMode.Create, FileAccess.Write))
            {
              ...
              do
              {
                ...
                totalread += bytesRead;
                double Percentagedownloaded = (((double)totalread / (double)contentLength) * 100);
                if (_VMVersionHelper != null)
                {
                  _VMVersionHelper.DownloadValue = string.Format("{0}%", ((int)Percentagedownloaded).ToString());
                }
        ...
        {
          DownloadComplete(statusCode, localFilePath);
        }
      }
      catch (System.Exception error)
      {
