...
namespace MyAgencyVault.BusinessLibrary
{
    ...
    public class PayorIncomingSchedule
    {
        ...
        public static PolicyToolIncommingShedule GetBasicScheduleOnSettingsID(Guid settingsID)
        {
            PolicyToolIncommingShedule schedule = new PolicyToolIncommingShedule();
            try
            {
                using (SqlConnection con = new SqlConnection(DBConnection.GetConnectionString()))
                {
                     using (SqlCommand cmd = new SqlCommand("usp_getIncomingScheuleOnSettingsID", con))
                     {
                         cmd.CommandType = System.Data.CommandType.StoredProcedure;
                         cmd.Parameters.AddWithValue("@scheduleID", settingsID);
                         con.Open();
            ...
            {
                ActionLogger.Logger.WriteLog("GetScheduleOnSetingsID ex: " + ex.Message, true);
                throw ex;
            }
            return schedule;
        }
        ...
        public static double GetGradedPercentOnRange(double totalPremium)
        {
            ...
            ActionLogger.Logger.WriteLog("GetGradedPercentOnRange request - " + totalPremium, true);
            try
            {
                using (SqlConnection con = new SqlConnection(DBConnection.GetConnectionString()))
                {
            ...
            {
                ActionLogger.Logger.WriteLog("GetGradedPercentOnRange ex: " + ex.Message, true);
                throw ex;
            }
        ...
        public static void SaveNonGradedSchedule(PayorIncomingSchedule schedule, Guid? policyId = null)
        {
            ...
            {
                ActionLogger.Logger.WriteLog("SaveNonGradedSchedule:Exception occurs while Save with schedule" + schedule + " " + ex.Message, true);
                throw ex;
            }
        }
        public static void DeletePolicySchedule(Guid policyId)
        ...
        public static void DeleteSchedule(Guid ScheduleID)
        {
            try
            {
                ActionLogger.Logger.WriteLog("DeleteSchedule ScheduleID" + ScheduleID, true);
                using (SqlConnection con = new SqlConnection(DBConnection.GetConnectionString()))
                {
