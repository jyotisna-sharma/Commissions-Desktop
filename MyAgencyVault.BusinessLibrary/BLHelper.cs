...
namespace MyAgencyVault.BusinessLibrary
{
    public static class BLHelper
    {
        ...
        {
            ActionLogger.Logger.WriteLog("getCompTypeIdByName req: " + value, true);
            ...
            try
            {
                ...
                ActionLogger.Logger.WriteLog("getCompTypeIdByName onjList- " + onjList.ToStringDump(), true);

                CompType objComp = onjList.Where(p => p.Names.ToLower() == value.Trim().ToLower()).FirstOrDefault();
                if (objComp != null)
                {
                    ActionLogger.Logger.WriteLog("getCompTypeIdByName object found: " + objComp.ToStringDump(), true);
                    if (objComp.IncomingPaymentTypeID != null)
                    {
                        CompTypeId = Convert.ToInt32(objComp.IncomingPaymentTypeID);
                    }
                }
                ActionLogger.Logger.WriteLog("getCompTypeIdByName result: " + CompTypeId, true);

            }

            catch (Exception ex)
            {
                ActionLogger.Logger.WriteLog("getCompTypeIdByName ex: " + ex.Message, true);
            }
