...
namespace MyAgencyVault.BusinessLibrary
{
    ...
    public class PostUtill
    {
        ...
        public static PostProcessReturnStatus PostStart(PostEntryProcess _PostEntryProcess, Guid DeuEntryId, Guid RepostNewDeuEntryId, Guid UserId, UserRole _UserRole, PostEntryProcess _ActualPostEntryStatus, string strUnlinkClient, string Operation, bool isInvoiceEdited = false, bool isUnlinkCall = false)
        {
            ...
            try
            {
                ...
                else if (_PostEntryProcess == PostEntryProcess.Delete)
                {
                    ...
                    //Commented by Acme - Left for follow up service 
            ...
            catch (Exception ex)
            {
                ...
                //Added by Jyotisna - May 15,2020
                //_PostProcessReturnStatus.IsComplete = false;
                //_PostProcessReturnStatus.ErrorMessage = "error in post";
            }
        ...
        public static void RemoveClient(Guid ClientId, Guid LicenseeId)
        {
            ...
            ActionLogger.Logger.WriteLog("RemoveClient started: ClientID - " + ClientId + ", LicenseeID: " + LicenseeId, true);
            ...
            try
            {
                ...
                if (num > 0)
                {
                    ActionLogger.Logger.WriteLog("RemoveClient ClientID - " + ClientId + ", LicenseeID: " + LicenseeId + ": policies found, returning....", true);
                    return;
                }
                ...
                ActionLogger.Logger.WriteLog("RemoveClient ClientID - " + ClientId + ", LicenseeID: " + LicenseeId + ": client deleted", true);
            }
            catch (Exception ex)
            {
        ...
        public static DEUFields FillDEUFields(Guid DeuEntryId, bool IsInvoiceEdited = false)
        {
                    ...
                    if (_UniqueIndenties.Count > 0)
                    {
                        ...
                        if (deufields.SearchedPolicyList.Count() == 1)
                        {
                            ...
                            if (string.IsNullOrEmpty(deufields.SearchedPolicyList.FirstOrDefault().ClientName))
                            {
                                ...
                                {
                                    ActionLogger.Logger.WriteLog("FillDEUFields:Exception occurs while processing for deufields DeuEntryId: " + DeuEntryId + ex.Message, true);
                                  //  throw ex;
                                }
                            }
            ...
            catch (Exception ex)
            {
                ...
                if (ex.InnerException != null)
                {
                    ActionLogger.Logger.WriteLog("Inner issue in FillDEUFields:  " + ex.InnerException, true);
                }
              //  throw ex;
            }
        ...
        static void SendmailIfOutgoingMismatch(Guid PaymentEntryID, Guid PolicyId, bool IsPaymentToHO, DateTime? InvoiceDate, Guid LicenseeId)
        {
            try
            {
                if (!CheckIsIncomingPaymentEqualsOutgoing(PaymentEntryID))
                {
                    ...
                    MailServerDetail.sendMailtodev(to, "Commissions Alert: Outgoing payments mismatch from desktop app", body);
                }
            }
            catch (Exception ex)
            {
        ...
        public static void EntryInPolicyOutGoingPayment(bool IsPaymentToHO, Guid PaymentEntryID, Guid PolicyId, DateTime? InvoiceDate, Guid LicenseeId)
        {
            ...
            catch (Exception ex)
            {
                ...
                body += "\nInvoiceDate: " + InvoiceDate;
                body += "\nLicenseeId: " + LicenseeId;
                MailServerDetail.sendMailtodev("deudev@acmeminds.com", "Commissions Alert: Exception adding outgoing payments ", body);
                //throw ex;
            }
        }
            ...
            try
            {
                ...
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
                    Guid UserCredID = (from f in DataModel.UserCredentials
                                       ...
                                       select f).Where(f => f.IsHouseAccount == true && f.RoleId == 3).FirstOrDefault().UserCredentialId;
                    return UserCredID;
        ...
        {
            ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries request: deuFields - " + deuFields.ToStringDump() + ", basicInfoProcess: " + _BasicInformationForProcess.ToStringDump(), true);
            ...
            try
            {
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
                    ...
                    {
                        ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries entry started: PaymentEntryID- " + PolicyEnteryID, true);
                        ...
                        ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries entry progress: Stmt done " + PolicyEnteryID, true);
                        ...
                        ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries entry progress: policy done " + PolicyEnteryID, true);
                        ...
                        ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries entry progress: Total Payment done " + PolicyEnteryID, true);
                        ...
                        ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries entry progress: PostStatus done " + PolicyEnteryID, true);
                        ...
                        ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries entry progress: AddToPolicyPaymentEntries done " + PolicyEnteryID, true);
                    }
                    DataModel.SaveChanges();
                    ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries entry progress: DataModelSaveChanges done " + PolicyEnteryID, true);
                    if (deuFields.DeuData != null && deuFields.DeuData.PolicyId == null)
                    {
                        MailServerDetail.sendMailtodev("deudev@acmeminds.com", "PolicyId save null in database for  paymentEntry", "PolicyId save null from paymentEntryId: " + PolicyEnteryID + "PolicyId:" + _BasicInformationForProcess.PolicyId);
                    }
            ...
            {
                ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries exception:" + ex.Message, true);

                if (ex.InnerException != null)
                {
                    ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries :" + ex.InnerException.Message, true);
                    ActionLogger.Logger.WriteLog("EntryInPoicyPamentEntries :" + ex.InnerException.ToStringDump(), true);
                }
                //if (deuFields.DeuData != null && deuFields.DeuData.PolicyId == null)
                //{
                MailServerDetail.sendMailtodev("deudev@acmeminds.com", "Exception saving payment in POlicyPaymentEntries table", "Exception saving payment - paymentEntryId: " + PolicyEnteryID + "PolicyId:" + _BasicInformationForProcess.PolicyId + ", ex: " + ex.Message);
                //}
             //   throw ex;
            }
        ...
        public static Guid CreateNewPendingPolicy(DEUFields deuFields, string strUnlinkClient)
        {
            ActionLogger.Logger.WriteLog("CreateNewPendingPolicy: processing begins for creating new pending policy" + deuFields.ToStringDump(), true);
            Guid NewPendingPolicyID = Guid.NewGuid();
            try
            {
                ...
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
                    ...
                    if (_policy == null)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            ActionLogger.Logger.WriteLog("CreateNewPendingPolicy, exception in assigning number/client:" + ex.StackTrace, true);
                            if (ex.InnerException != null)
                            {
                                ActionLogger.Logger.WriteLog("CreateNewPendingPolicy _policy.PolicyClientId :" + _policy.PolicyClientId.ToString() + " " + "Inner Exception:" + ex.InnerException.Message, true);
                            }
                       //     throw ex;
                        }
                        ActionLogger.Logger.WriteLog("CreateNewPendingPolicy _policy.PolicyClientId :" + _policy.PolicyClientId.ToString(), true);
                    ...
                    {
                        ActionLogger.Logger.WriteLog("CreateNewPendingPolicy1 Exception :" + ex.Message, true);
                        ActionLogger.Logger.WriteLog("CreateNewPendingPolicy1 Exception :" + ex.StackTrace, true);
                        if (ex.InnerException != null)
                        {
                            ActionLogger.Logger.WriteLog("CreateNewPendingPolicy1: Inner exception :" + " " + ex.InnerException.Message, true);
                        }
            ...
            {
                ActionLogger.Logger.WriteLog("CreateNewPendingPolicy exception:" + ex.Message, true);
                ActionLogger.Logger.WriteLog("CreateNewPendingPolicy exception:" + ex.StackTrace, true);
                if (ex.InnerException != null)
                {
                    ActionLogger.Logger.WriteLog("CreateNewPendingPolicy: Inner exception :" + " " + ex.InnerException.Message, true);
                }
               // throw ex;
            }
            return NewPendingPolicyID;
        ...
        private static DLinq.Client CreateNewClientForPendingPolicy(DEU Deu, DLinq.CommissionDepartmentEntities DataModel, Guid licenceId)
        {
            ...
            if (Deu.ClientID != null && Deu.ClientID != Guid.Empty)
            {
                ...
                if ((from f in DataModel.Clients where f.ClientId == ClientId select f).FirstOrDefault() == null)
                {
                    ...
                    {
                        ActionLogger.Logger.WriteLog("CreateNewClientForPendingPolicy exception occurs :" + ex.Message, true);
                        ActionLogger.Logger.WriteLog("CreateNewClientForPendingPolicy exception occurs :" + ex.StackTrace, true);
                        if (ex.InnerException != null)
                        {
                            ActionLogger.Logger.WriteLog("CreateNewClientForPendingPolicy:processing failure: exception" + ex.InnerException.Message, true);
                        }
                       // throw ex;
                    }
                }
            ...
            else
            {
                ...
                {
                    ActionLogger.Logger.WriteLog("CreateNewClientForPendingPolicy processing failure exception:" + ex.Message, true);
                    ActionLogger.Logger.WriteLog("CreateNewClientForPendingPolicy processing failure exception:" + ex.StackTrace, true);
                    if (ex.InnerException != null)
                    {
                        ActionLogger.Logger.WriteLog("CreateNewClientForPendingPolicy:processing failure: Inner exception" + ex.InnerException.Message, true);
                    }
                   // throw ex;
                }
            }
        ...
        {
            ActionLogger.Logger.WriteLog("CreateNewClientForUnlikingPolicy process starts:Deu" + Deu.PolicyId, true);
            ...
            {
                ActionLogger.Logger.WriteLog("CreateNewClientForUnlikingPolicy process failure exception:" + ex.Message, true);
                ActionLogger.Logger.WriteLog("CreateNewClientForUnlikingPolicy process fails with  exception:" + ex.StackTrace, true);
                if (ex.InnerException != null)
                {
                    ActionLogger.Logger.WriteLog("CreateNewClientForUnlikingPolicy:processing failure:Inner exception" + ex.InnerException.Message, true);
                }
             //   throw ex;
            }
        ...
        public static PolicyDetailsData GetPolicy(Guid SePolicyID)
        {
            ...
            {
                ActionLogger.Logger.WriteLog("GetPolicy:Exception occurs while processing" + ex.Message, true);
              //  throw ex;
            }
        ...
        public static bool CheckForIncomingScheduleVariance(PolicyPaymentEntriesPost _TempPolicyPaymentEntriesPost, decimal? Premium)
        {
            ...
            try
            {
                ...
                else if (_PolicySchedule == PolicySchedule.Basic)
                {
                    ...
                    {
                        ActionLogger.Logger.WriteLog("Incoming schedule is null, returning", true);
                    ...
                    if (_incomingSchedule.Mode == Mode.Custom) //Handle custom case 
                    {
                        if (_incomingSchedule.CustomType == CustomMode.Graded)
                        {
                            ...
                            int year = _PolicyPaymentEntriesPost.CreatedOn.Year;
                            DateTime startDate = new DateTime(year, 1, 1);
                            DateTime endDate = _PolicyPaymentEntriesPost.CreatedOn;
                            ...
                            if (_incomingSchedule.GradedSchedule != null && _incomingSchedule.GradedSchedule.Count > 0)
                            {
                                ...
                                if (sum > maxVal)
                                {
                                    ActionLogger.Logger.WriteLog("Sum is greated than maxValue, so using max Value ", true);
                                    record = _incomingSchedule.GradedSchedule.Where(x => x.To == maxVal).FirstOrDefault();
                                }
                                else
                                {
                                    ActionLogger.Logger.WriteLog("Sum is lesser than maxValue, so NOT using max Value ", true);
                                }
                                ...
                                {
                                    ActionLogger.Logger.WriteLog("Graded schedule found null ", true);
                                }
                            }
                            else
                            {
                        ...
                        else //Non-graded
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    year = (int)Math.Ceiling((_PolicyPaymentEntriesPost.CreatedOn - Convert.ToDateTime(_PolicyLearnedField.Effective)).TotalDays / 365.25D);
                                    year = (year <= 0) ? 1 : year;
                                }
                            }
                            catch (Exception ex)
                            {
                                ActionLogger.Logger.WriteLog("Exception in getting polic age: " + ex.Message, true);
                            }
                            ...
                            if (_incomingSchedule.NonGradedSchedule != null)
                            {
                                ...
                                int maxYear = _incomingSchedule.NonGradedSchedule.OrderByDescending(x => x.Year).FirstOrDefault().Year;
                                year = (year > maxYear) ? maxYear : year;

                                double? expectedPercent = _incomingSchedule.NonGradedSchedule.Where(x => x.Year == year).FirstOrDefault().Percent;
                            ...
                            {
                                ActionLogger.Logger.WriteLog("POlicy non graded schedule is null: ", true);
                            }
                        }
        ...
        {
            ActionLogger.Logger.WriteLog("IsUseFirstYear request ", true);
            ...
            try
            {
                ...
                if (EffDate != null)
                {
                    ActionLogger.Logger.WriteLog("IsUseFirstYear effDate not null ", true);
                    ...
                    if ((AddEffDate.Value <= _invoicedate) && _invoicedate >= EffDate.Value)
                    {
                        ...
                        ActionLogger.Logger.WriteLog("IsUseFirstYear renewal true ", true);
                    }
                    else
                    {
                        Flag = FirstYrRenewalYr.FirstYear;
                        ActionLogger.Logger.WriteLog("IsUseFirstYear true ", true);
                    }
                }
                else
                {
                    // Acme commented Nov 29, 2019, as following if is wrong 
                    //ActionLogger.Logger.WriteLog("IsUseFirstYear effDate null ", true);
                    //if (FirstYr != Renewal)
                    //{
                ...
                ActionLogger.Logger.WriteLog("IsUseFirstYear flag: " + Flag, true);
            }
            catch (Exception ex)
            {
        ...
        public static FirstYrRenewalYr IsUseFirstYearForOutGoing(DateTime? _invoicedate, DateTime? OriginalEffectiveDate, Guid PolicyId)
        {
            ...
            try
            {
                ...
                {
                    Flag = FirstYrRenewalYr.FirstYear; // Removed none case from here - nov 28, 2019

                }
                ActionLogger.Logger.WriteLog("IsUseFirstYearForOutGoing for policy:" + PolicyId + ", " + Flag, true);
            }
            catch (Exception ex)
            {
        ...
        {
            ActionLogger.Logger.WriteLog("CheckForOutgoingScheduleVariance starts ", true);
            double TruncationError = Convert.ToDouble(Masters.SystemConstant.GetKeyValue("TruncationError"));
            ...
            try
            {
                ...
                {
                    Flag = false;
                    ActionLogger.Logger.WriteLog("Policy schedule false - flag false ", true);
                }
                else if (_PolicySchedule == PolicySchedule.Basic)
                {
        ...
        public static DateTime? GetOldestInvoiceDate(Guid PolicyId)
        {
            ...
            {
                ActionLogger.Logger.WriteLog("GetOldestInvoiceDate:Exception occurs while processing" + ex.Message, true);
                //throw ex;
