...
namespace MyAgencyVault.EmailFax
{
    public class EmailFax
    {
        ...
        private void SendMessage()
        {
            try
            {
                ActionLogger.Logger.WriteLog("Send message started: To - " + To, true);
                ...
                try
                {
                    ...
                    ActionLogger.Logger.WriteLog("Send message Success : To - " + To, true);
                }

                catch (Exception ex)
                {
                    ActionLogger.Logger.WriteLog("Exception in Send message 1 : To - " + To + ", ex: " + ex.Message, true);
                   // ex.ToString();
                }
            }
            catch (Exception ex)
            {
                ActionLogger.Logger.WriteLog("Exception in Send message 2: To - " + To + ", ex: " + ex.Message, true);
               // ex.ToString();
            }
        }
    }
