...
namespace MyAgencyVault.ImportToolManualStart
{
    public partial class Form1 : Form
    {
        ...
        private void startProcessing()
        {
            ...
            StartStopWindowService();          
            StartImportToolService objStartImportToolService = new StartImportToolService();

            foreach (string strFilePath in lstFilesName.Items)
            {
                if (!string.IsNullOrEmpty(strFilePath.Trim()))
                {
                    objStartImportToolService.StartFolderWatcher(strFilePath);
                }
            }

            //After complication of manaully import start the window servive 
            StartStopWindowService();
        ...
        private void LoadFiles()
        {
           ...
           // string strserver = @"D:\ImportPayorTool\Processing";
            ...
            //Get All file from processing folder
            var strFiles = Directory.GetFiles(strFilePath, "*.*", SearchOption.AllDirectories).Where(s => supportedExtensions.Contains(System.IO.Path.GetExtension(s).ToLower()));
            //Load into the list box
            foreach (string FilePath in strFiles)
            {
                string strValue = FilePath;
                if (!string.IsNullOrEmpty(strValue.Trim()))
                {
                    lstFilesName.Items.Add(FilePath);
                }
            }
            //Enabled and disabled button
            if (lstFilesName.Items.Count > 0)
            {
        ...
        //Srat and stop import tool windows service
        private void StartStopWindowService()
        {
            try
            {
                //Get the import tool windows service 
                ServiceController service = new ServiceController("ImportToolService");
                if (service != null)
                {
        ...
        public class StartImportToolService
        {
            ...
            public void StartFolderWatcher(string FilePath)
            {
                try
                {
                    ...
                    //foreach (string FilePath in lstBox.Items)
                    //{
                ...
                try
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            ActionLogger.Logger.WriteImportLog("Phrases can't be found in any payor/template . ", true);
                            MoveToTempfolder();
                            MoveToUnSuccesfullfolder();
                        }
        ...
        private void lstFilesName_SelectedIndexChanged(object sender, EventArgs e)
        {
